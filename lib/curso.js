 ...
 *
 * En las fichas hay valores escritos de muchas formas: "funcionario",
 * "Funcionario de carrera", "FUNCIONARIO DE CARRERA", "interino con
 * vacante"... Antes se comparaba el texto exacto y todo lo que no
 * encajaba caía en el cajón de 1 día, dejando a funcionarios de carrera
 * con un solo día de libre disposición. Aquí se reduce a tres casos.
 *
 * Devuelve: 'carrera' | 'interino_vacante' | 'interino_sin_vacante' | 'otro'
 */
export function normalizaContrato(tipoContrato) {
  const t = (tipoContrato || '')
    .toString().trim().toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '');   // sin acentos

  if (!t) return 'otro';
  if (t.includes('funcionari')) return 'carrera';        // funcionario, funcionaria, funcionario de carrera
  if (t.includes('interin')) {
    // "sin vacante" manda: si no lo dice, se entiende con vacante
    if (t.includes('sin vacante')) return 'interino_sin_vacante';
    if (t.includes('vacante')) return 'interino_vacante';
    return 'interino_sin_vacante';
  }
  return 'otro';
}

/**
 * Días de libre disposición que le corresponden a una persona.
 *
 * ÚNICO sitio donde se calcula. Antes había dos copias, una en la
 * pantalla del profesor y otra en la de dirección, y no daban lo mismo:
 * la de dirección no miraba la edad para el CANOSO.
 *
 * El CANOSO se tiene por dos vías y basta con cumplir una: 55 años o
 * más, o 18 o más de servicio.
 */
export function calcularDiasDLD(tipoContrato, antiguedadCuerpo, anioNacimiento = null) {
  const edad = anioNacimiento ? new Date().getFullYear() - parseInt(anioNacimiento, 10) : null;
  const porEdad = edad !== null && !isNaN(edad) && edad >= 55;
  const porServicio = (parseInt(antiguedadCuerpo, 10) || 0) >= 18;
  const tieneDerechoCanoso = porEdad || porServicio;

  const clase = normalizaContrato(tipoContrato);
  let moscosos;
  if (clase === 'carrera' || clase === 'interino_vacante') moscosos = 3;
  else if (clase === 'interino_sin_vacante') moscosos = 2;
  else moscosos = 1;

  const canosos = tieneDerechoCanoso ? 1 : 0;
  return { moscosos, canosos, total: moscosos + canosos, tieneDerechoCanoso, porEdad, porServicio, edad, clase };
}
