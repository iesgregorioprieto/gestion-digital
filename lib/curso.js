...
export function limiteDLD(fecha, cfg, tipoDld = null) {
  ...
  const info = fecha ? esDiaLectivo(fecha, cfg) : { lectivo: true, motivo: null };
  ...
  const esLectivo = tipoDld === 'no_lectivo' ? false : info.lectivo;

  const limite = esLectivo
    ? (plantilla > 60 ? 4 : plantilla > 40 ? 3 : plantilla > 20 ? 2 : 1)
    : Math.floor(plantilla / 3);

  return { limite, esLectivo, motivo: info.motivo, plantilla };
}
