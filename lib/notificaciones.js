...
/**
 * Quién recibe los avisos de bajas médicas.
 *
 * Las bajas NO se envían desde el portal a Inspección Médica. El aviso
 * llega a estos dos, que son quienes deciden qué se manda y a dónde
 * desde sus propios correos. Así ningún dato médico sale del centro por
 * una vía automática.
 */
export const AVISOS_BAJAS = [
  'jjds04@educastillalamancha.es',   // José María Díaz-Cano, director
  'ssam17@educastillalamancha.es',   // Sebastián Aldavero, jefe de estudios
];
...
export async function avisarDireccion(cliente, tipo, datos) {
  ...
  if (destinos.length === 0) {
    // Si nadie tiene el cargo puesto en su ficha, el aviso no sale y
    // nadie se entera de que no ha salido. Al menos que quede escrito.
    console.error(`[aviso ${tipo}] sin destinatarios: ninguna ficha con rol_gestion 'director'`);
    return;
  }
