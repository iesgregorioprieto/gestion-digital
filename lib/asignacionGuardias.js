 ...
 *
 * Se cubren tanto las horas de clase como las de GUARDIA: si al que
 * falta le tocaba guardia, el centro se queda con un guardia menos y
 * hay que reponerlo. La diferencia es que una guardia huérfana no tiene
 * grupo ni aula, así que da igual quién entre: no se mira el
 * departamento, solo el sector y la rotación.
 */
...
export function asignacionesDeHora({
  ...
  const elegir = (hueco, esGuardia) => {
    ...
    if (propios.length > 0) {
      propios.sort((a, b) =>
        // En una guardia huérfana no hay alumnos de la materia: el
        // desempate por departamento no pinta nada.
        (esGuardia ? 0 : mismoDepto(b, hueco) - mismoDepto(a, hueco))
  ...
  for (const hueco of pendientes) {
    ...
    const esGuardia = (detalle.tipo || '').toLowerCase().includes('guardia');
    ...
    const r = elegir(hueco, esGuardia);
    ...
    salida.push({
      ...
      hora: h,
      esGuardia,
      grupo: esGuardia ? (detalle.grupo || hueco.sector) : (detalle.grupo || clase.grupo || ''),
      aula: esGuardia ? '' : (clase.aula || ''),
      materia: esGuardia ? '' : (clase.materia || ''),
      ...
      motivo: r ? (esGuardia ? `${r.motivo} · guardia huérfana` : r.motivo)
                : 'sin cubrir: nadie disponible',
    });
  }
