...
export function asignacionesDeHora({
  ...
  function libresParaApoyo(asignadosAbrev, sectorSolicitante) {
    // Iterar TODOS los sectores: un sector con 1 ausente puede tener
    // 3 profesores mas disponibles para cubrir.
    const libres = [];
    ...
    for (const sector of sectores) {
      guardiasDeSector(sector).forEach(p => {
        ...
        // Primer intento: clave normalizada exacta
        let completo = (profesores || []).find(pf =>
          claveAbreviatura(pf.apellidos, pf.nombre) === key
        );
        // Segundo intento: solo el primer apellido, por si Delphos lo abrevia distinto
        if (!completo) {
          const primerApellido = key.split(' ')[0];
          completo = (profesores || []).find(pf => {
            const pfKey = claveAbreviatura(pf.apellidos, pf.nombre);
            return pfKey.startsWith(primerApellido) && pfKey.length < primerApellido.length + 5;
          });
        }
        libres.push({
          abrev: p,
  ...
  for (const sectorSup of Object.keys(porSector)) {
    ...
    for (const aus of porSector[sectorSup]) {
      ...
      const guardiasHora = aus.horas.filter(h => horaCoincide(h.hora, hora) && h.tipo === 'guardia');

      // Guardia huérfana: el ausente tenía guardia él mismo.
      // Se preasigna al mejor candidato disponible por orden de prioridad.
      for (const gHora of guardiasHora) {
        ...
        if (libres.length > 0) {
          ...
          asignaciones.push({
            ausencia: aus,
            clase: gHora,
            cubre: { ...primero, tipo: 'guardia_sector', alternativas: libres.slice(1, 6) },
          });
        }
      }

      // Clases que hay que cubrir
