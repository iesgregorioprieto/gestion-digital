...
export function asignacionesDeHora({
  ...
  /**
   * DOS PASADAS, y el orden importa mucho.
   *
   * Antes se recorrían las ausencias una a una, por orden alfabético, y
   * a cada una se le daba el mejor candidato disponible EN ESE MOMENTO.
   * Eso tenía una consecuencia grave: si a una ausencia de FP no le
   * quedaba nadie de su departamento, se llevaba a los de GENERAL
   * (escalón 1). Cuando después le tocaba el turno a una ausencia de
   * GENERAL, ya no quedaba ningún general libre y acababa cubierta por
   * alguien de FP -- habiendo habido generales de guardia esa hora.
   *
   * Y dependía del orden alfabético, así que unos días pasaba y otros no.
   *
   * La regla del centro es tajante: NUNCA se coge a un profesor de FP
   * para cubrir una ausencia de GENERAL mientras quede algún general de
   * guardia, y al revés igual. Para garantizarlo hay que asignar primero
   * TODOS los que se pueden cubrir con su propio sector, y solo después
   * repartir los que se han quedado sin nadie de lo suyo.
   */
  const asignado = new Map();   // hueco.profesorId → resultado de elegir()

  // Pasada 1: solo el propio sector. Nadie sale de su sector todavía.
  for (const hueco of pendientes) {
    const r = elegir(hueco, { soloPropioSector: true });
    if (r) {
      ocupadosAhora.add(r.elegido.id);
      asignado.set(hueco, r);
    }
  }

  // Pasada 2: a los que han quedado sin cubrir se les busca fuera.
  for (const hueco of pendientes) {
