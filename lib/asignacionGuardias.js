...
export function prepararHuecos(faltas, profesores, opciones = {}) {
  ...
  for (const falta of faltas || []) {
    ...
    const horasClase = marcadas.length > 0
      ? porHoras
      : expandirDesdeHorario({ profesor: prof, horarios, dia, indice, bloques: declaradas });

    /**
     * 6ª HORA EN FP: NO SE CUBRE.
     *
     * Si falta un profesor de FP a última hora, sus alumnos se van a casa
     * (son mayores de 16 y tienen la autorización de sus familias). No
     * queda nadie en el aula, así que no se manda a ningún guardia: los de
     * GENERAL solo cubren a GENERAL. A esa hora FP ni siquiera tiene
     * guardias en el cuadrante.
     *
     * Sigue constando que el profesor no está a 6ª (horasAusente).
     */
    const esFP = esSectorFP(sectorDe(prof));
    const horas = esFP
      ? horasClase.filter(h => !horaCoincide(h.hora, '6'))
      : horasClase;
