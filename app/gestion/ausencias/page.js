...
<table class="datos">
  ...
  <tr><td>Motivo</td><td>${a.subtipo ? etiquetaMotivo(a.subtipo) : (a.tipo === 'prevista' ? 'Prevista' : 'Imprevista')}</td></tr>
              ...
              return (
                <div key={a.id} style={{ backgroundColor: vencida ? '#fffbfb' : 'white', borderRadius: 12, padding: 16, marginBottom: 10, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', borderLeft: `4px solid ${vencida ? '#b91c1c' : est.color}` }}>
                  ...
                  <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {/* Antes este botón solo salía si la ausencia estaba
                        pendiente, así que en cuanto el profesor la
                        justificaba dirección se quedaba sin poder
                        revisarla ni añadir observaciones. */}
                    <button onClick={() => { setAusenciaGestion(a); setComentarioJust(a.comentario_secretario||''); setObsDirectivo(a.observaciones_directivo||''); }} style={{ padding: '7px 14px', borderRadius: 7, border: '1.5px solid #93c5fd', backgroundColor: '#dbeafe', color: '#1e40af', fontSize: 13, cursor: 'pointer', fontWeight: 600 }}>📋 Gestionar</button>
